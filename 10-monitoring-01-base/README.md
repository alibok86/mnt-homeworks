...
## Ответ 1
  ### **1. Системные метрики (инфраструктура)**

-   **CPU Usage**  (%) – средняя и пиковая загрузка процессора.
-   **Memory Usage**  (RAM, %) – потребление оперативной памяти.
-   **Disk Space**  (свободное место в %) – особенно на разделах, где хранятся отчеты.
-   **Disk I/O**  (read/write latency, throughput) – если отчеты часто записываются.
-   **Network Traffic**  (входящий/исходящий) – если отчеты могут скачиваться по HTTP.

### **2. Метрики приложения (HTTP-сервис)**

-   **HTTP Request Rate**  (RPS) – количество запросов в секунду
-   **HTTP Response Time**  (latency, p50/p95/p99) – время обработки запросов.
-   **HTTP Error Rate**  (5xx, 4xx) – количество ошибок.
- 
### **3. Бизнес-метрики**
-   **Количество сгенерированных отчетов**  (в единицу времени).
-   **Средний размер отчета**  (в МБ/КБ).
...
-   **SLI** : Расчет по ответу от приложения клиенту. _Успех_: Запрос должен отдавать `200`ые или `300`ые коды в течение `5` секунд. _Неудача_: нарушение любого условия (ответ или время)
...
## Ответ 3
В таком случае, раз разработчики хотят видеть ошибки, то они должны реализовать в приложении систему файлового логирования, а еще лучше дополнительно доработать под работу с **Sentry**. Можно предложить построить бюджетные схемы сбора логов:
...
## Ответ 7
![Текст описания](https://github.com/alibok86/mnt-homeworks/blob/MNT-video/10-monitoring-01-base/3.png)
...
## Ответ 8
![Текст описания](https://github.com/alibok86/mnt-homeworks/blob/MNT-video/10-monitoring-01-base/1.png)
...
## Ответ 9
![Текст описания](https://github.com/alibok86/mnt-homeworks/blob/MNT-video/10-monitoring-01-base/2.png)
